...
hemoglobin_data$fish <- sample(rep(1:80, length.out = nrow(hemoglobin_data)), replace = FALSE)
...
mean_hemoglobin <- aggregate(hemoglobin ~ rate + method, data = hemoglobin_data, FUN = mean)
...
mean_hemoglobin_rate3_methodA <- mean_hemoglobin$hemoglobin[which(mean_hemoglobin$rate == 3 & mean_hemoglobin$method == "A")]

# Estimate mean hemoglobin value for each rate
mean_hemoglobin_rate <- aggregate(hemoglobin ~ rate, data = hemoglobin_data, FUN = mean)
```
...
friedman_test_result <- friedman.test(cream_data$acidity, cream_data$starter, cream_data$batch)
